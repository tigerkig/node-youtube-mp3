...
async function getInfo(url) {
    var usefullPart = url.split("&list=")[0];
    ...
    var fullTitle = info.videoDetails.title;
...
function clearText(text) {
    var source = typeof text === "string" || text instanceof String ? text : "";
...
async function downloadThumbnail(info) {
    ...
    var filenameOB = await download.image(options);
...
async function preprocessSong(info) {
    ...
    var imagePath = await downloadThumbnail(info);
